...
int convert_matrix(char* infile, char* outfile_a) {
    FILE *fp;
    SpMatrix_t *A;
    Vector_t *b, *x;
    char dmy[16];
    ...
    while(1) {
        ...
        printf("INFO: Reading vector %s...\n", dmy);
        sprintf(outfile_v, "%s.bin", dmy);
        ...
        SpMatrix_save_vector(outfile_v, b);
    }
